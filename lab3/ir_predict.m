...
A = [X; -X; C];
b = [y+epsilon; -y+epsilon; d]; %%KYSTYN -y+epsilon
...
for i = 1:k
    [betalow, flow, exitcode, lambda] = glpk (Xp(i,:), A, b, [], [], ctype, vartype, sense, param); %KYSTYN lb ub
    actlow=[];
    if exitcode > 0
      return
    endif
    ...
    [betahigh, fhigh, exitcode, lambda] = glpk (-Xp(i,:), A, b, [], [], ctype, vartype, sense, param);
    actupp=[];
    if exitcode > 0
      return
    endif
